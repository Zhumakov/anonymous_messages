...
from source.exceptions import MessageCreateHTTPException, MessageVerifyHTTPException
...
async def verify_and_get_message(from_user_uid: str, reply_to_message: int) -> Message:
    ...
    if not primary_message or from_user_uid != primary_message.to_user_uid:
        raise MessageVerifyHTTPException
...
async def send_message_and_notification(
    ...
    result = await MessagesService.insert_into_table(
    ...
    if not result:
        raise MessageCreateHTTPException
...
async def get_messsages_on_category(
    ...
    match category:
        ...
        case "accepted":
            results = await MessagesService.get_all(to_user_uid=user_uid)
        case "reply":
